...
string createObjectCodeFormat34()
{
  ...
  { //Immediate
    ...
    if (if_all_num(tempOperand) || ((SYMTAB[make_pair(tempOperand, "*")].isPresent == 'y') && (SYMTAB[make_pair(tempOperand, "*")].relative == 0)
                                    /**/
                                    && (CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].isPresent == 'n')) /****/)
    ...
    else if (SYMTAB[make_pair(tempOperand, "*")].isPresent == 'n' || CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].isPresent == 'y')
    {

      if (CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].isPresent != 'y' || halfBytes == 3)
      {
        writeData = "Line " + to_string(lineNumber);
        if (halfBytes == 3 && CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].isPresent == 'y')
        {
          writeData += " : Invalid format for external reference " + tempOperand;
        }
        else
        {
          writeData += " : Symbol doesn't isPresent. Found " + tempOperand;
        }
        writeToFile(errorFile, writeData);
        objcode = intToStringHex(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 1, 2);
        objcode += (halfBytes == 5) ? "100000" : "0000";
      ...
      if (SYMTAB[make_pair(tempOperand, "*")].isPresent == 'y' && CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].isPresent == 'y')
      {
        objcode = intToStringHex(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 1, 2);
        objcode += "100000";
    ...
    {
      int operandAddress = stringHexToInt(SYMTAB[make_pair(tempOperand, "*")].address) + stringHexToInt(BLOCKS[BLocksNumToName[SYMTAB[make_pair(tempOperand, "*")].blockNo]].startAddress);
  ...
  {
    string tempOperand = operand.substr(1, operand.length() - 1);
    if (tempOperand.substr(tempOperand.length() - 2, 2) == ",X" || SYMTAB[make_pair(tempOperand, "*")].isPresent == 'n' /*****/ || CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].isPresent == 'y' /*****/)
    { //Error handling for Indirect with index based
      ...
      if (CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].isPresent != 'y' || halfBytes == 3)
      {
        ...
        { /*****/
          writeData += " : Symbol doesn't isPresent.Index based addressing not supported with Indirect addressing ";
      /*****/ } /*****/
      writeToFile(errorFile, writeData);
      ...
      /*
      writeData = "Line "+to_string(lineNumber);
      writeData += (SYMTAB[make_pair(tempOperand,"*")].isPresent=='n')?": Symbol doesn't isPresent":" Index based addressing not supported with Indirect addressing";
      writeToFile(errorFile,writeData);
      objcode = intToStringHex(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode)+2,2);
      objcode += (halfBytes==5)?"100000":"0000";
      return objcode;*/
    ...
    writeData = "Line: " + to_string(lineNumber);
    writeData += "Can't fit into program counter based or base regStruct based addressing.";
    writeToFile(errorFile, writeData);
    objcode = intToStringHex(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 2, 2);
    objcode += (halfBytes == 5) ? "100000" : "0000";
  ...
  { //Literals
    ...
    if (tempOperand == "*")
    {
      ...
      modificationRecord += "M" + intToStringHex(stringHexToInt(LITTAB[tempOperand].address) + stringHexToInt(BLOCKS[BLocksNumToName[LITTAB[tempOperand].blockNo]].startAddress), 6) ;
      modificationRecord += intToStringHex(6, 2);
      modificationRecord += '\n';
    }

    if (LITTAB[tempOperand].isPresent == 'n')
    {
      writeData = "Line " + to_string(lineNumber) + " : Symbol doesn't isPresent. Found " + tempOperand;
      writeToFile(errorFile, writeData);
    ...
    int operandAddress = stringHexToInt(LITTAB[tempOperand].address) + stringHexToInt(BLOCKS[BLocksNumToName[LITTAB[tempOperand].blockNo]].startAddress);
    program_counter = address + stringHexToInt(BLOCKS[BLocksNumToName[blockNumber]].startAddress);
    program_counter += (halfBytes == 5) ? 4 : 3;
  ...
  { 
    ...
    if (SYMTAB[make_pair(tempOperand, "*")].isPresent == 'n' /*****/ || CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].isPresent == 'y' /*****/)
    {
      /*****/
      if (CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].isPresent != 'y' || halfBytes == 3)
      { /*****/
...
void writeTextRecord(bool lastRecord = false)
{
  ...
  else
  {
    ...
    {
      //Write current record if isPresent
      if (currentRecord.length() > 0)
      {
        writeData = intToStringHex(currentRecord.length() / 2, 2) + currentRecord;
        writeToFile(objectFile, writeData);
...
void pass2()
{
  ...
  while (opcode != "END")
  {
    ...
    while (opcode != "END" && opcode != "CSECT")
    {
      ...
      if (!isComment)
      {
        if (OPTAB[getRealOpcode(opcode)].isPresent == 'y')
        {
          ...
          else if (OPTAB[getRealOpcode(opcode)].format == 2)
          {
            ...
            { //If not two operand i.e. a
              ...
              else if (REGTAB[operand1].isPresent == 'y')
              {
              ...
              else
              {
                ...
                writeData = "Line: " + to_string(lineNumber) + " Invalid regStruct name";
                writeToFile(errorFile, writeData);
              }
            }
            else
            { //Two operands i.e. a,b
              if (REGTAB[operand1].isPresent == 'n')
              {
              ...
              else if (REGTAB[operand2].isPresent == 'n')
              {
                objectCode = OPTAB[getRealOpcode(opcode)].opcode + "00";
                writeData = "Line: " + to_string(lineNumber) + " Invalid regStruct name";
                writeToFile(errorFile, writeData);
        ...
        else if (opcode == "WORD")
        {
          ...
          if (operand[0] - '0' >= 0 && operand[0] - '0' < 10)
        ...
        {
          if (SYMTAB[make_pair(operand, "*")].isPresent == 'y')
          {
            base_register_value = stringHexToInt(SYMTAB[make_pair(operand, "*")].address) + stringHexToInt(BLOCKS[BLocksNumToName[SYMTAB[make_pair(operand, "*")].blockNo]].startAddress);
            nobase = false;
          }
          else
          {
            writeData = "Line " + to_string(lineNumber) + " : Symbol doesn't isPresent. Found " + operand;
            writeToFile(errorFile, writeData);
          }
          objectCode = "";
        }
...
int main()
{
  ...
  {
    writestring += it.first.first + ":-\t" + "name:" + it.second.name + "\t|" + "address:" + it.second.address + "\t|" + "relative:" + intToStringHex(it.second.relative) + " \n";
  }
